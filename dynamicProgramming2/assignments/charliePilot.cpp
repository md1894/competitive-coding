...
A plane crew consists of two pilots - a captain and his assistant. A captain must be older than his assistant. 
...
if he decides to spend some time to make the optimal 
(i.e. the cheapest) arrangement of pilots in crews.
Input
...
Each of those N lines contains two integers separated by a space character, X i Y, 1 ≤ Y < X ≤ 100,000, 
a salary as a captain (X) and a salary as an assistant (Y).
Output
...
/*
note that, here variable x denotes the difference of number of assistant-captain
or assistant-pilot
*/

// memoized
int charlie(int* captain, int* assistant, int n, int x, int** dp){
       if(n == 0)
            return 0;
        ...
        if(x == 0){
            dp[n][x] = assistant[0] + charlie(captain+1,assistant+1,n-1,x+1,dp);
            return dp[n][x];
        }
        else if(x == n){
            dp[n][x] = captain[0] + charlie(captain+1,assistant+1,n-1,x-1,dp);
            return dp[n][x];
        }
        else{
            int op1 = assistant[0] + charlie(captain+1,assistant+1,n-1,x+1,dp);
            int op2 = captain[0] + charlie(captain+1,assistant+1,n-1,x-1,dp);
            dp[n][x] = min(op1,op2);
            return dp[n][x];
        }
}
...
int charlie(int* captain, int* assistant, int n){
    ...
    int ans = charlie(captain, assistant, n, 0, dp);
    for(int i = 0; i < n; i++){
        delete [] dp[i];
    }
    delete [] dp;
