/*
After the release of Avengers, Ironman and Thor got into a fight and Ironman challenged Thor 
to find out the number of numbers between 1 and n which are divisible by any of the prime numbers less than 20. 
...
/* logic is similar to the subset problem */
int main(){
    int t;
    cin >> t;
    ...
    while(t--){
        ...
        for(ll i = 1; i <= subsets; i++){
            /* count number of bits which are one */
            ll setBitsCount = __builtin_popcount(i);
            ll denominator = 1;
            for(int j = 0; j < 8; j++){
                /* if the bit is one then only it is considered */
                if(i&(1<<j)){
