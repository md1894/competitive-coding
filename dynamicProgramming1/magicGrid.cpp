...
A dragon at a cell (i,j) takes away |S[i][j]| strength points from him, 
and a potion at a cell (i,j) increases Harry's strength by S[i][j]. 
If his strength drops to 0 or less at any point during his journey, Harry dies, 
...
T cases follow. Each test case consists of R C in the first line followed by 
the description of the grid in R lines, each containing C integers. 
...
int solve(int** grid, int row, int col){
    ...
    for(int i = col-3; i >= 0; i--){
        int ans = dp[row-1][i+1] - grid[row-1][i+1];
        if(ans <= 0){
            ans = 1;
        }
        dp[row-1][i] = ans;
    }
    /* filling up the last column */
    for(int i = row-3; i >= 0; i--){
        int ans = dp[i+1][col-1] - grid[i+1][col-1];
        if(ans <= 0){
            ans = 1;
        }
        dp[i][col-1] = ans;
