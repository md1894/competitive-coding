...
long dfs(vector<long>* edges, long st, long* banana, unordered_set<long>& visited){
    visited.insert(st);
    long ans = banana[st];
    for(long i = 0; i < edges[st].size(); i++){
        long neighbor = edges[st][i];
        if(visited.count(neighbor) == 0){
            ans += dfs(edges,neighbor,banana,visited);
        }
    }
    return ans;
}

long maxPrize(vector<long>* edges, long n, long m, long* banana){
    unordered_set<long> visited;
    long max = -1;
    for(long i = 0; i < n; i++){
        if(visited.count(i) == 0){
            long currentAns = dfs(edges,i,banana,visited);
            if(currentAns > max){
                max = currentAns;
            }
        }
...
int main(){
    ...
    while(t--){
        long n, m;
        cin >> n >> m;
        vector<long>* edges = new vector<long>[n];
        for(long i = 0; i < m; i++){
            long x, y;
            cin >> x >> y;
        ...
        long* banana = new long[n];
        for(long i = 0; i < n; i++){
            cin >> banana[i];
        }
        cout << maxPrize(edges,n,m, banana) << endl;
        delete [] banana;
        delete [] edges;
    }
    return 0;
